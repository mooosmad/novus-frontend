...
export default function TabNavigation({ activeTab, onTabChange }: TabNavigationProps) {
  ...
  return (
    <motion.div 
      className="bg-white rounded-full p-1 inline-flex shadow-blue shadow-lg"
      ...
      {tabItems.map((item, index) => (
        <motion.button
          ...
          className={`relative px-6 py-3 rounded-full font-medium transition-all duration-300 whitespace-nowrap ${
            activeTab === item.id
              ? 'text-white bg-blue-950 shadow-sm'
        ...
        >
          ...
          {activeTab === item.id && (
            <motion.div
              className="absolute inset-0 bg-blue text-white rounded-full shadow-sm"
              layoutId="activeTab"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
